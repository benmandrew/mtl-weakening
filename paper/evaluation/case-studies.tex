\subsection{Feasibility of Approach in Real-World Case Studies (RQ2)}\label{subsec:case-studies}

\begin{table}[t]
\caption{Interval-weakenable requirements in \fret{} case studies.}
% \marienote{there should be 42 requirements in the aircraft engine controller }
\label{table:weakenable-reqs-case-studies}
\centering
\begin{tabular}{ |l|r|r| } 
\hline
\textbf{Case study} & \textbf{Total requirements} & \textbf{Weakenable requirements} \\\hline
Mechanical lung ventilator~\cite{farrell2024} & 121 & 63 \\\hline
Autonomous drone~\cite{sheridan2025} & 62 & 19 \\\hline
Lift-plus-cruise aircraft~\cite{pressburger2023} & 49 & 30 \\\hline
Aircraft engine controller~\cite{farrell2022} & 42 & 0 \\\hline
Inspection rover~\cite{bourbouh2021} & 15 & 2 \\\hline
Grasping for debris removal~\cite{farrell2022a} & 20 & 0 \\\hline
Robotic patterns~\cite{vazquez2024} & 28 & 9 \\\hline
LMCP challenges~\cite{mavridou2020} & 73 & 7 \\\hline
\textbf{Total} & \textbf{410} & \textbf{130} \\\hline
\end{tabular}
\end{table}
%
\begin{figure*}[t]
    ...
    % \marienote{more detail please: The first one is taken from an autonomous drone use case which references the time taken for the control loop to complete and the second one describes the behaviour of the inspiratory pause button in a mechanical lung ventilator system. Both of these are taken from systems that are fully implemented and operational in real world settings.}
\end{figure*}
%
...
\end{equation}
%
% \marienote{Are there consequences of the control loop taking  24 time units to complete? What kind of degraded operation might cause this? (we can email Leandro if needed)}
%
