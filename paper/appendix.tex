...
\Function{\weakenreleasedirect($\psi_l$, $\psi_r$, $[a,b]$, $\pi$, $t$)}{
...
\Function{\weakenuntilright($C$, $\phi$, $[a,b]$, $\psi\bintemporal{\Iorig}\psi'$, $\pi$, $t$)}{
    $\bfin \gets \min(b, \rightidx_\pi(a))$ \\
    $intervals \gets [\;]$ \\
    \For{$i \gets a$ \KwTo $\bfin$}{
...
\SetAlgoNlRelativeSize{-1}
\caption{Weakening within $\mathcal{R}$ on the right}
\Function{\weakenreleaseright($C$, $\phi$, $[a,b]$, $t$)}{
    $\bfin \gets \min(b, \rightidx_\pi(a))$ \\
    $intervals \gets [\;]$ \\
    \For{$i \gets a$ \KwTo $\bfin$}{
        $I \gets \weakenaux(C, \psi\bintemporal{I}\psi',\pi, t + i)$\\
        \If{I $=$ None}{
            \Return $None$
        }
        append $I$ to $intervals$\\
